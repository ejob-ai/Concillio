  ...
  return (
    <html lang={lang} data-app-root data-theme={ssrTheme} class={ssrTheme === 'dark' ? 'dark' : undefined}>
      <head>
        ...
        {(() => { let head: any = {}; try { head = (c.get as any)?.('head') || {} } catch {} let canonical = ''; let altSv = ''; let altEn = ''; try { const url = new URL(c.req.url); const pathname = url.pathname || ''; if (pathname.startsWith('/docs/roller')) { canonical = 'https://concillio.pages.dev/docs/roller'; altSv = canonical; altEn = canonical; } else if (pathname.startsWith('/docs/lineups')) { canonical = 'https://concillio.pages.dev/docs/lineups'; altSv = canonical; altEn = canonical; } else { url.searchParams.set('lang', lang); canonical = url.toString(); const uSv = new URL(canonical); uSv.searchParams.set('lang','sv'); altSv = uSv.toString(); const uEn = new URL(canonical); uEn.searchParams.set('lang','en'); altEn = uEn.toString(); } } catch {} return (<>

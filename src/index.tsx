...
// Inline checkout fallback handler to avoid 404s in prod if router mount is skipped
const checkoutInlineHandler = jsxRenderer(({ c }: { c: any }) => {
  const url = new URL(c.req.url)
  const planKey = ((url.searchParams.get('plan') || 'starter') as 'free'|'starter'|'pro'|'legacy')
  const plan = (PLANS as any)[planKey] || PLANS.starter
  try { c.set('head', { title: 'Checkout – Concillio', description: 'Välj din plan och kom igång på minuter.' }) } catch {}
  c.header('Cache-Control', 'public, max-age=900, must-revalidate')
  return (
    <main class="container mx-auto py-8">
      ...
      {'priceUSD' in plan ? (
        <p class="subtle mb-6">Price: <strong>${(plan as any).priceUSD.toFixed(2)}</strong> / mo</p>
...
app.get('/checkout', checkoutInlineHandler)

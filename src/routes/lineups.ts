...
router.get('/api/lineups/presets', async (c) => {
  ...
  const rs = await DB.prepare(`SELECT p.id, p.name, p.is_public, json_group_array(json_object('role_key', r.role_key, 'weight', r.weight, 'position', r.position)) AS roles

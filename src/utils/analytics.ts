...
export async function writeAnalytics(DB: D1Database, data: any) {
  ...
  // Try insert with legacy "ts" column first (for DBs where it is NOT NULL)
  try {
    await DB.prepare(
      `INSERT INTO analytics_council (event, label, path, ts_client, ts_server, created_at, req_id, role, minutes_id, total_latency_ms, latency_ms, model, prompt_tokens, completion_tokens, cost_usd, ts)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      evSan,
      labelSan,
  ...
  // Fallback: insert without legacy column
  await DB.prepare(
